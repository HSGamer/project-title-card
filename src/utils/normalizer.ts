...
export function normalizeColor(colorStr: string | undefined, fallback: string): string {
...
export function parseCssStyle(styleStr: string | undefined): Record<string, string> {
...
export function parseFontWeight(weightStr: string | undefined, fallback: TitleFontWeight): TitleFontWeight {
  if (!weightStr) return fallback;
  const clean = weightStr.trim().toLowerCase();
  if (clean === 'bold' || clean === 'bolder') return '700';
  if (clean === 'normal' || clean === 'lighter' || clean === 'light') return '400';
...
export function parseFontFamily(familyStr: string | undefined, fallback: string): string {
  if (!familyStr) return fallback;
  const clean = familyStr.replace(/;/g, '').trim();
  const lower = clean.toLowerCase();
  if (lower.includes('monospace') || lower.includes('mono') || lower.includes('courier') || lower.includes('consolas')) {
  ...
  if (lower.includes('arial') || lower.includes('helvetica') || lower.includes('sans-serif') || lower.includes('system-ui') || lower.includes('inter')) {
    return 'Inter, system-ui, -apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, sans-serif';
  }
  return clean;
}

export function extractGradientStops(defsStr: string): { start?: string; middle?: string; end?: string; isRadial: boolean } {
  const result: { start?: string; middle?: string; end?: string; isRadial: boolean } = {
...
export function normalizeCardOptions(raw: unknown): CardOptions {
  ...
  const rawBorder = typeof rawObj.border === 'object' && rawObj.border !== null
    ...
    : null;
  const rawTitleFont = typeof rawObj.titleFont === 'object' && rawObj.titleFont !== null
    ? (rawObj.titleFont as Record<string, unknown>)
    : null;
  const rawDescFont = typeof rawObj.descriptionFont === 'object' && rawObj.descriptionFont !== null
  ...
  const legacyBgCss = parseCssStyle(typeof rawObj.backgroundStyle === 'string' ? rawObj.backgroundStyle : undefined);
  const legacyTitleCss = parseCssStyle(typeof rawObj.titleStyle === 'string' ? rawObj.titleStyle : undefined);
  const legacyDescCss = parseCssStyle(typeof rawObj.descriptionStyle === 'string' ? rawObj.descriptionStyle : undefined);
  const legacyDefs = typeof rawObj.defs === 'string' ? rawObj.defs : '';

  // Determine format (support legacy & alias)
  let format: GenerateType = 'card';
  if (rawObj.generateType === 'widecard' || rawObj.generateType === 'readme-banner') {
    format = 'widecard';
  } else if (rawObj.generateType === 'widescreen' || rawObj.generateType === 'social-preview') {
    format = 'widescreen';
  } else if (rawObj.generateType === 'badge' || rawObj.generateType === 'compact-badge') {
    format = 'badge';
  }
  ...
  const bgType: BackgroundType =
    rawBg?.type === 'solid' || rawBg?.type === 'gradient' || rawBg?.type === 'glass' || rawBg?.type === 'image'
      ? (rawBg.type as BackgroundType)
      : isGradientFromLegacy
      ? 'gradient'
      : 'solid';

  const extractedStops: { start?: string; middle?: string; end?: string; isRadial?: boolean } =
  ...
  const gradStart =
    typeof rawBg?.gradientStart === 'string'
      ? rawBg.gradientStart
      : extractedStops.start || '#ea580c';

  const gradEnd =
    typeof rawBg?.gradientEnd === 'string'
      ? rawBg.gradientEnd
      : extractedStops.end || '#7c3aed';

  const gradMiddle =
    typeof rawBg?.gradientMiddle === 'string'
      ? rawBg.gradientMiddle
      : extractedStops.middle;

  const gradDir: GradientDirection =
    rawBg?.gradientDirection === 'to-r' ||
  ...
  const bgImageUrl = typeof rawBg?.imageUrl === 'string' ? rawBg.imageUrl : undefined;
  const bgImageOpacity = typeof rawBg?.imageOpacity === 'number' ? rawBg.imageOpacity : 1;
  const bgOverlayColor = typeof rawBg?.overlayColor === 'string' ? rawBg.overlayColor : undefined;
  const bgOverlayOpacity = typeof rawBg?.overlayOpacity === 'number' ? rawBg.overlayOpacity : 0.5;

  // 2. Border parsing
  const borderColor =
    typeof rawBorder?.color === 'string'
      ? rawBorder.color
      : legacyBgCss.stroke
  ...
  const borderStyle: BorderStyle =
    rawBorder?.style === 'dashed' || rawBorder?.style === 'dotted' || rawBorder?.style === 'none'
      ? rawBorder.style
  ...
  const titleFontFamily =
    typeof rawTitleFont?.fontFamily === 'string'
      ? rawTitleFont.fontFamily
      : legacyTitleCss['font-family']
      ? parseFontFamily(legacyTitleCss['font-family'], baseDefault.titleFont.fontFamily)
      : baseDefault.titleFont.fontFamily;

  const titleFontWeight: TitleFontWeight =
    rawTitleFont?.fontWeight === '400' ||
    ...
    rawTitleFont?.fontWeight === '900'
      ...
      ? parseFontWeight(legacyTitleCss['font-weight'], baseDefault.titleFont.fontWeight)
      : baseDefault.titleFont.fontWeight;

  const titleFontSize =
    typeof rawTitleFont?.fontSize === 'number'
  ...
  const titleLetterSpacing =
    typeof rawTitleFont?.letterSpacing === 'number'
  ...
  const descColor =
    typeof rawDescFont?.color === 'string'
      ? rawDescFont.color
      : legacyDescCss.fill
      ? normalizeColor(
          legacyDescCss.fill,
          'descriptionFont' in baseDefault ? baseDefault.descriptionFont.color : '#94a3b8'
        )
  ...
  const descFontFamily =
    typeof rawDescFont?.fontFamily === 'string'
      ? rawDescFont.fontFamily
      : legacyDescCss['font-family']
      ? parseFontFamily(
          legacyDescCss['font-family'],
          'descriptionFont' in baseDefault ? baseDefault.descriptionFont.fontFamily : baseDefault.titleFont.fontFamily
        )
      : 'descriptionFont' in baseDefault
  ...
  const descFontWeight: DescriptionFontWeight =
    ...
    rawDescFont?.fontWeight === '700'
      ...
      ? (parseFontWeight(legacyDescCss['font-weight'], '500') as DescriptionFontWeight)
      : '500';

  const descFontSize =
    typeof rawDescFont?.fontSize === 'number'
  ...
  const descLineHeight =
    typeof rawDescFont?.lineHeight === 'number'
      ? rawDescFont.lineHeight
      : 1.3;

  const descOpacity =
    typeof rawDescFont?.opacity === 'number'
      ? rawDescFont.opacity
