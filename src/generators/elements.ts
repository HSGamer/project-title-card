...
export function renderImage(
  ...
  const img = draw.image(imageConfig.url).size(width, height).move(
    pos.x,
    pos.y,
  );
  img.attr({
  ...
  if (imageConfig.shape === "circle") {
    const clipId = `${clipIdPrefix}_circle_${Math.floor(pos.x)}_${
      Math.floor(pos.y)
    }`;
    ...
    const clipId = `${clipIdPrefix}_rounded_${Math.floor(pos.x)}_${
      Math.floor(pos.y)
    }`;
    const clip = defs.element("clipPath").attr("id", clipId);
