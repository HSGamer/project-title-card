...
export function generateWidescreen(options: WidescreenCardOptions): SVGSVGElement {
  ...
  if (options.layoutStyle === 'centered') {
    ...
    if (hasImage) {
      renderImage(draw, options.image, { x: imgX, y: imgY }, smallImg, smallImg, 'wsLogo');
    }
    ...
    if (hasImage) {
      ...
      renderImage(draw, options.image, { x: imgX, y: imgY }, bannerImgSize, bannerImgSize, 'wsLogo');
    }
    ...
    if (hasImage) {
      ...
      renderImage(draw, options.image, { x: imgX, y: imgY }, imgSize, imgSize, 'wsLogo');
